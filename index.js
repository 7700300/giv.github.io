...
var ioTitle=0;
var ioVar=0;
var fight_from_castle=0;
iTempData=new Array();
try {var pAccount=top.frames[5].platinumAccount;} catch (e) {var pAccount=0;};
...
var mbPower=0;
var mbSniper=0;
var mbPereman=127930;

var mbStone=247962;
var mbFireBall=247961;
var mbArmor=247959;
var mbFreeze=247960;
var mbCurse=247956;
...
var xhr = (window.XMLhttpsRequest && !window.ActiveXObject) ? function() {
return new window.XMLhttpsRequest();} : function() {
try {return new window.ActiveXObject('Microsoft.XMLhttps');} catch(e) {return null}};
xhr = xhr();
if (!xhr) return null;
...
if(a==0) {
actIframe.location="room.html";
byid("t").innerHTML=""
+"<a href=\"#\" onclick=\"AddJS(1,'scripts/ItemOperationCity2.js');byid('t').innerHTML=LoadImg;\" "
+"style=\"margin-left:50%;\">[Раздеть/Одеть]</a>";
}
if(a==1) {
actIframe.location="smith.html";
byid("t").innerHTML=""
+"<a href=\"#\" onclick=\"AddJS(1,'scripts/ioRepair.js');byid('t').innerHTML=LoadImg;\" "
+"style=\"margin-left:60%;\">[Чинить всё]</a>";
}
if(a==2) {
byid("t").innerHTML=""
+"<a href=\"#\" onclick=\"actIframe.location='privsmith.html?unick='+d.nk+'';\" "
+"style=\"margin-left:60%;\">[В Кузницу]</a>";
}
if(a==3) {
byid("t").innerHTML=""
+"<a href=\"#\" onclick=\"AddJS(1,'scripts/ioStorage.js');byid('t').innerHTML=LoadImg;\" "
...
if(a==3 && top.frames[3].document.getElementById("messages").getElementsByTagName("span").length!=0) {
for(var i=0; i<top.frames[3].document.getElementById("messages").getElementsByTagName("span").length; i++) {
var chat_msg=top.frames[3].document.getElementById("messages").getElementsByTagName("span")[i].id;
var chatReg=/row(\d+)/;
var chatRowArray=chatReg.exec(chat_msg);
// check-messages
var breq=top.frames[3].document.getElementById("row"+chatRowArray[1]).innerHTML;
var reg=new RegExp("("+ME.nk+"|На форпост)","g");
if(reg.test(breq) && !persIframe.chatRowArray[chatRowArray[1]]) {
...
if(a==4 && top.frames[3].document.getElementById("messages").getElementsByTagName("span").length!=0) {
for(var i=0; i<top.frames[3].document.getElementById("messages").getElementsByTagName("span").length; i++) {
var chat_msg=top.frames[3].document.getElementById("messages").getElementsByTagName("span")[i].id;
var chatReg=/row(\d+)/;
var chatRowArray=chatReg.exec(chat_msg);
...
var reg=new RegExp("("+ME.nk+"|На форпост)","g");
if(reg.test(breq) && !persIframe.chatRowArray[chatRowArray[1]]) {
persIframe.chatRowArray[chatRowArray[1]]=1;
}}
}
}
...
+"float:left;cursor:hand;\" id="+id+">"+txt+"</div></a>";
}
}
...
top.frames[2].ready_mb=0;
top.frames[2].cast_load_count++;
};
// END-READYMB
...
if(move_round>0 && UNBS[ME.id] && UNBS[ME.id].flg!=8 &&
   persIframe.document.CrDemand.abMoveСlick.value==1) {
...
if(!actIframe.document.getElementById("control_msg")) {
// msg-log
var element=actIframe.document.getElementsByTagName("b")[1];
var text=actIframe.document.createElement("span");
text.id="control_msg";
...
+"<span style=background-color:yellow;color:black;>СОЗДАТЬ_ЗАЯВКУ</span>";
actIframe.document.getElementById("control_msg").innerHTML=control_text;
var chform=actIframe.document.forms["cd"];
chform.submit();
actIframe.document.body.appendChild(chform);
}
...
as_audio=""
+"<audio autoplay>"
+"<source src=\"https://"+hostname_oil+"/audio/achtung.mp3\" type=\"audio/mpeg\">"
+"</audio>";
document.getElementById("rjob").innerHTML=""
...
+"return false;\">"
 ...
 if(t==55) {
if (fight_from_castle == 1){
 fight_from_castle = 0;
 document.getElementById("fight_fr_castle").innerHTML=""
+"<img border=0 src=https://"+hostname_oil+"/img/arrow/checkbox-a.gif width=48 height=22 onclick=MainSwitch(55);>";
} else {
fight_from_castle = 1;
 document.getElementById("fight_fr_castle").innerHTML=""
+"<img border=0 src=https://"+hostname_oil+"/img/arrow/checkbox.gif width=48 height=22 onclick=MainSwitch(55);>";	
}
}
...
+"<td>"
+"<span style=\"display:block;width:48;height:22;"
+"background:url(https://apeha.ru/img/smode-3.gif) no-repeat right center;\" id=\"fight_fr_castle\">"
+"<img border=0 src=https://"+hostname_oil+"/img/arrow/checkbox.gif width=48 height=22 onclick=MainSwitch(55);>"
+"</span>"
+"</td>"	
+"</tr>"
...
var div=actIframe.document.createElement("div");
div.id="status";
div.style.top="10px";
div.style.left="10px";
...
div.style.zIndex="1";
div.innerHTML=txt;
actIframe.document.body.appendChild(div);
}

function LocSite(elem,tag,txt) {
if(elem=="title") {
for(i=0; i<actIframe.document.getElementsByTagName(tag).length; i++) {
if(actIframe.document.getElementsByTagName(tag)[i].title==txt) {
return true;
}}}
if(elem=="value") {
for(i=0; i<actIframe.document.getElementsByTagName(tag).length; i++) {
if(actIframe.document.getElementsByTagName(tag)[i].value==txt) {
return true;
}}}
if(elem=="name") {
for(i=0; i<actIframe.document.getElementsByTagName(tag).length; i++) {
if(actIframe.document.getElementsByTagName(tag)[i].name==txt) {
return true;
}}}
}
...
break; // ноги
case 3: SwitchAttack(1);ubkick(0,0);ubkick(1,4);MakeTurn();
...
var ha_r=persIframe.byid("INJ_r").title;
var ha_l=persIframe.byid("INJ_l").title;
...
persIframe.document.CrDemand.act_castle.click();
return abAudio("achtung.mp3");
}
if(FLDY>3 && FLDX+FLDY!=27 && amountenemy>2 && ME.astral>5) { // astral-up
ChangeAstralLevel(0);
} // end-astral-up
if(amountenemy==1 && ME.lvl>7) { // limit
byid("logb").innerHTML="<span style=\"color:red;\">&#9674;Лимит врагов</span>";
if(persIframe.document.CrDemand.act_castle.value==1 && amountfriend==1) {
persIframe.document.CrDemand.act_castle.click();
persIframe.document.getElementById("t").innerHTML="НЕТ ИГРОКОВ. СТОП.";
}
return abAudio("call_message.mp3");
} // end-limit
top.frames[5].chclear();
top.frames[5].chclear();
if(!DEAD[ME.id] && UNBS[ME.id].flg!=8) AddJS(1,"scripts/jsgame_AddData.js");
return tmID[0]=setTimeout("autotest()",5555);
} // end-control

ab_limit_hp=persIframe.ab_limit_hp;
var canmove1=byid("buttons").style.visibility;
var rxy=-1;
var nxy=-1;
var onlyclon=0;
...
if(persIframe.document.CrDemand.clonsum.value==1 &&
...
if(logLastRoundID>=persIframe.document.CrDemand.abround.value) { // limit-raund
byid("logb").innerHTML=""
+"<span style=\"color:red;\">"
+"&#215;Лимит "+persIframe.document.CrDemand.abround.value+" раунд"
+"</span>";
abAudio("call_message.mp3");
return document.getElementById("ctrl_btn").value="ON";
} // end-limit-raund
...
var condition_hp=persIframe.condition_hp;
...
if(persIframe.document.CrDemand.abHP.value==1) {
byid("logb").innerHTML=""
+"<span style=\"color:red;\">"
+"&#215;Лимит HP"
+"</span>";
...
+"onclick=\"if(yes_mbCast!=0){move_round=0;yes_mbCast=1;"+btn_cast_1+"}\" "
...
+"onclick=\"if(yes_mbCast!=0){move_round=0;yes_mbCast=1;"+btn_cast_2+"}\" "
...
+"color:black;background-color:#E8EEEC;border:solid 1px black;padding:1px;cursor:pointer;position:absolute;\" "
+"onclick=\"if(yes_mbCast!=0){move_round=0;yes_mbCast=1;"+btn_cast_6+"}\" "
...
+"actIframe.location='fortunawheeldata_actUser-SpinWheel_1.html';","","")
+NewButton(0,"55px","#8A492F","#8A492F","#FFEEC0","gold","«Сумка»","ItemOperationCity(0);","","")
+(d.id==9215?
"<a href=\"javascript:void(0);\" onclick=\"actIframe.location='castle_room_1_cid_'+MyClan+'.html';\" "
+"oncontextmenu=\"actIframe.location='medroom_cid_'+MyClan+'.html';return false;\">"
...
+"actIframe.location='arena_room_1_bmode_3.html';","","")
+"</td>"
+"</tr>"
+"<tr>"
+"<td>"
+"<a href=\"javascript:void(0);\" onclick=\"ItemOperationCity(1);\" "
+"oncontextmenu=\"actIframe.location='jewelry.html?unick='+d.nk+'';"
...
+NewButton(0,"55px","#8A492F","#8A492F","#FFEEC0","gold","«Лавка»","actIframe.location='shop.html';","","")
+NewButton(0,"55px","#8A492F","#8A492F","#FFEEC0","gold","«Рынок»","actIframe.location='market.html';","","")
+NewButton(0,"50px","#8A492F","#8A492F","#FFEEC0","gold","«К&nbsp;магу»",""
+"actIframe.location='magiccasters.html?unick='+d.nk+'';","","")
+"</td>"
+"</tr>"
+"<tr>"
+"<td>"
+"<a href=\"javascript:void(0);\" onclick=\"actIframe.location='bank.html';\" "
+"oncontextmenu=\"actIframe.location='auctions.html';return false;\">"
+"<div onmouseover=\"info(0,this,'gold')\" onmouseout=\"endi(this,'#FFEEC0')\" "
...
+NewButton(0,"55px","#8A492F","#8A492F","#FFEEC0","gold","«Турнир»","actIframe.location='arenat.html';","","")
+"<a href=\"javascript:void(0);\" onclick=\""
+"actIframe.location='sawmill_mode_3.html';\" "
+"oncontextmenu=\""
+"actIframe.location='mine_mode_3.html';return false;\">"
+"<div onmouseover=\"info(0,this,'gold')\" onmouseout=\"endi(this,'#FFEEC0')\" "
...
var addObs=function() {
try {
var nform=actIframe.document.forms.length;
var y=actIframe.document.forms["f_ub"].ubl1.value;
...
if(!LocSite("value","input","Прервать работу") && 
   !actIframe.document.getElementById("buttons")) { // wheeluck
...
if(!actIframe.document.getElementsByTagName("META")[0] &&
   !LocSite("value","INPUT","html") && document.CrDemand.act_castle.value==1) { // meta
byid("t").innerHTML="html.META";
if(actIframe.document &&
   actIframe.document.getElementsByTagName &&
   actIframe.document.getElementById && actIframe.document.body) {
Indicator("red","<input type=button value=html>",25);
}
actIframe.location=med_room;
} // end-meta
var health="осталось";
...
element.parentNode.insertBefore(text, element);
// end-msg-log
AddJS(1,"scripts/auto_demand.js");
}
if(!LocSite("name","INPUT","Battle{vall}") && nform!=0) { // в лечебницу (от бандита)
actIframe.location="arena_room_1_bmode_3.html";
persIframe.setTimeout("top.frames[1].frames['channel_3'].location=med_room",1500);
}
...
var e3=document.all("dinjcell").innerHTML;
var xhl=new RegExp(health,"g");
var bad_loc=RoomReg[1].test(actIframe.location);
if(soclanList.length==1) bad_loc=/arena_room_1_bmode_1/.test(actIframe.location);
if(xhl.test(e3) && !bad_loc && fight_from_castle==0) { // в лечебницу
	actIframe.location=med_room;
}
if(xhl.test(e3) && !bad_loc && fight_from_castle==1) { //к замку
	actIframe.location=castle_room;
}
var e3=document.all("dinjcell").innerHTML;
var xhl=new RegExp(health,"g");
var bad_loc=RoomReg[0].test(actIframe.location);
if(soclanList.length==1) bad_loc=/arena_room_1_bmode_1/.test(actIframe.location);
...
+"var bad_event=/осталось/.test(top.frames[1].document.getElementById('dinjcell').innerHTML);"
+"if(bad_event) {"
+"document.getElementById('dinjcell2').innerHTML="
+"top.frames[1].document.getElementById('dinjcell').getElementsByTagName('td')[2].innerHTML;"
...
+"}"
+"setTimeout('msgBadEvent()',1500);";
actIframe.document.getElementsByTagName("head")[0].appendChild(script);
// end-msg-fun-log
var control_text=""
...
var itm=top.frames[5].document.forms["chat"].elements["filter"];
itm.value=253; // Сообщения комнаты
top.frames[5].rChat(3); // Сообщения комнаты
} // end-chat
}
if(a>0) {
if(ME.id==100214920) { // bb
mbClon=15687;
mbHP=15683;
mbStone=15684;
mbFireBall=15686;
...
soclanList=persIframe.soclanList;
}
if(a==0 && b==2) {
